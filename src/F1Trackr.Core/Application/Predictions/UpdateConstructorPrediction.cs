...
public sealed class UpdateConstructorPrediction
{
    public sealed record Command(
        GroupId GroupId,
        UserId UserId,
        IDictionary<ConstructorId, int> Positions) : ICommand<Result>;
    ...
    public sealed class CommandHandler : ICommandHandler<Command, Result>
    {
        ...
        public async Task<Result> HandleAsync(Command command, CancellationToken cancellationToken)
        {
            ...
            member.ConstructorPredictions.Clear();
            ...
            foreach (var (constructorId, position) in command.Positions)
            {
                if (!constructors.TryGetValue(constructorId, out var constructor))
                {
                    result.WithError(new ValidationError($"Constructor with ID {constructorId} not found."));
                    continue;
                }

                member.ConstructorPredictions.Add(new ConstructorPrediction(constructor, position));
            }
