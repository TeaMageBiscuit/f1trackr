...
const refresh = async () => {
  ...
  try {
    const result = await http.get<GroupMember>(`/api/admin/groups/${groupId}/members/${userId}`)
    if (result.success) {
      groupMember.value = result.value!

      const groupResult = await http.get<GroupOverview>(`/api/admin/groups/${groupId}`)
...
async function showConstructorPredictionDialog() {
  ...
  dialog.open(ConstructorPredictionDialog, {
    ...
    data: {
      endpoint: `/api/admin/groups/${groupMember.value!.groupId.value}/members/${groupMember.value!.userId.value}/constructors`,
...
async function showDriverPredictionDialog() {
  ...
  dialog.open(DriverPredictionDialog, {
    ...
    data: {
      endpoint: `/api/admin/groups/${groupMember.value!.groupId.value}/members/${groupMember.value!.userId.value}/drivers`,
      season: group.value?.season,
      drivers: drivers,
    },
    onClose: (options) => {
...
async function showRacePredictionDialog(round: number) {
  ...
  dialog.open(RacePredictionDialog, {
    ...
    data: {
      endpoint: `/api/admin/groups/${groupMember.value!.groupId.value}/members/${groupMember.value!.userId.value}/races/${round}`,
      season: group.value?.season,
...
<template>
  <GroupMemberPage
    :loading="loading"
    :can-edit="account.isAdmin!"
    :can-force-edit="account.isAdmin!"
    :member="groupMember"
    :races="races"
    @refresh="refresh"
    @back="goBack"
